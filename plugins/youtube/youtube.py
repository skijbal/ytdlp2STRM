...
def filter_and_modify_bandwidth(m3u8_content):
    ...
    for i in range(len(lines)):
        ...
        if line.startswith("#EXT-X-MEDIA:"):
            ...
            # Always have a fallback audio line in case English isn't present
            fallback_audio_line = fallback_audio_line or line

            langv = (attrs.get("LANGUAGE", "") or "").lower()
            namev = (attrs.get("NAME", "") or "").lower()

            # Scoring: strongly prefer English, otherwise keep something playable
            score = 0
...
def direct(youtube_id, remote_addr):
    ...
    if '-audio' not in youtube_id:
        ...
        command = [
            'yt-dlp',
            '-f', 'bestaudio[language^=en]/bestaudio',
            '--get-url',
            '--no-warnings',
...
def bridge(youtube_id):
    ...
    def generate():
        ...
        if config["sponsorblock"]:
            command = ['yt-dlp', '--no-warnings', '-o', '-', '-f', 'bestvideo*+bestaudio[language^=en]/bestvideo*+bestaudio/best', '--sponsorblock-remove',  config['sponsorblock_cats'], '--restrict-filenames', s_youtube_id]
        else:
            command = ['yt-dlp', '--no-warnings', '-o', '-', '-f', 'bestvideo*+bestaudio[language^=en]/bestvideo*+bestaudio/best', '--restrict-filenames', s_youtube_id]
        Youtube().set_cookies(command)
        Youtube().set_language(command)
        Youtube().set_proxy(command)
        if '-audio' in youtube_id:
            command[5] = 'bestaudio[language^=en]/bestaudio'
...
def download(youtube_id):
    ...
    if config["sponsorblock"]:
        command = ['yt-dlp', '-f', 'bv*+ba[language^=en]/bv*+ba/best', '-o',  os.path.join(temp_dir, '%(title)s.%(ext)s'), '--sponsorblock-remove',  config['sponsorblock_cats'], '--restrict-filenames', s_youtube_id]
    else:
        command = ['yt-dlp', '-f', 'bv*+ba[language^=en]/bv*+ba/best', '-o',  os.path.join(temp_dir, '%(title)s.%(ext)s'), '--restrict-filenames', s_youtube_id]
    Youtube().set_cookies(command)
    Youtube().set_language(command)
    Youtube().set_proxy(command)
    if '-audio' in youtube_id:
        command[2] = 'bestaudio[language^=en]/bestaudio'
